...
export function PlaceholdersAndVanishInput({
  ...
  const vanishAndSubmit = () => {
    ...
    if (inputValue && inputRef.current) {
      ...
      // Call onVanish if provided
      onVanish && onVanish();
      
      // Create a synthetic event for onSubmit
      const syntheticEvent = {
        target: { querySelector: () => ({ value: inputValue }) },
        preventDefault: () => {}
  ...
  // Expose vanishAndSubmit method to parent components if needed
  const triggerVanish = () => {
    if (!animating && inputRef.current?.value) {
      vanishAndSubmit();
    }
  };
